...
4. Do not add buttons, links, tabs, toggles, menus, floating actions, or button-like decorations outside the interaction map, except controls rendered and owned by the Eazo SDK. Mark each intentionally exposed SDK-owned control at its product usage site with a stable literal `data-eazo-sdk-control="<official-control-name>"`.
